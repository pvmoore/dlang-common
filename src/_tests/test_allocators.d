...
void testSparseArrayIndexes() {
    writefln("----------------------------------------------------------------");
    writefln(" Testing SparseArrayIndexes");
    writefln("----------------------------------------------------------------");
    ...
    }
    {
        ...
        auto s = new SparseArrayIndexes(128);
        assert(s.isEmpty());
        assert(s.numItems() == 0);
    ...
    foreach(i; 6..25) {
        ulong capacity = 1 << i;
        auto ss = new SparseArrayIndexes(capacity);
